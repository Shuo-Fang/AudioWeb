...
public class TokenHolder {
     ...
     */
    private void parseJson(String result) throws IOException {
    	JSONObject json = JSONObject.parseObject(result);
        if (!json.containsKey("access_token")) {
            // 返回没有access_token字段
            throw new IOException("access_token not obtained, " + result);
        }
        if (!json.containsKey("scope")) {
            // 返回没有scope字段
            throw new IOException("scope not obtained, " + result);
        }
        if (!json.getString("scope").contains(scope)) {
            throw new IOException("scope not exist, " + scope + "," + result);
        }
